...
export const PortfolioView: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'Tümü' | 'Web' | 'Mobil' | 'Oyun' | 'Tasarım'>('Tümü');
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            ...
            >
              <div className="flex gap-2 bg-secondary/50 p-1 rounded-xl backdrop-blur-sm border border-white/5">
                {['Tümü', 'Web', 'Mobil', 'Oyun', 'Tasarım'].map((cat) => (
                  <button
                    key={cat}
                    onClick={() => setFilter(cat as any)}
                    className={`px-4 py-2 rounded-lg text-xs font-medium transition-all relative whitespace-nowrap ${
                      filter === cat ? 'text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'
                    }`}
                  >
                    {filter === cat && (
                      <motion.div 
